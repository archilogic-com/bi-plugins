...
export class FloorPlan extends React.Component<Props, State> {
  ...
  async initFloorPlan(container) {
    if (!container) return
    const fpe = new FloorPlanEngine({ container, options: startupOptions })
    const isFloorPlanLoaded = await fpe.loadScene(this.state.floorID, {
  ...
  updateFloorPlan(floorPlan: FloorPlanEngine, state) {
    if (!floorPlan) return
    if (state.spaceIDs && state.spaceIDs.length === 0) return
  ...
  render() {
    ...
    return (
      <div id="app">
        ...
        <div id="floorplan" className={this.props.isLegendEnabled ? 'withLegend' : 'fullHeight'}>
          {this.state.publishableToken && this.state.floorID ? null : (
            <div id="token-id-warning">
              <p>Please provide Publishable Token and Floor ID in the Format Pane</p>
            </div>
          )}
        </div>
      </div>
